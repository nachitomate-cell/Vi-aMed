...
const ModalPrestacion: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className={`bg-white rounded-2xl shadow-2xl w-full flex flex-col border border-slate-200 transition-all duration-300 ${isExpanded ? 'max-w-5xl h-[95vh]' : 'max-w-2xl max-h-[92vh]'}`}>
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 flex-shrink-0">
          <div>
            <h2 className="text-base font-bold text-slate-800">
              {inicial.codigo ? 'Editar prestación' : 'Nueva prestación'}
            </h2>
          ...
          <div className="flex items-center gap-1">
            <button 
              ...
              className="p-2 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-[#0E7490] transition-colors"
              title={isExpanded ? "Reducir tamaño" : "Agrandar cuadro"}
            >
              {isExpanded ? (
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 9V4.5M9 9H4.5M9 9L3 3M15 15v4.5M15 15h4.5M15 15l6 6" /></svg>
              ) : (
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5v-4m0 4h-4m4 0l-5-5" /></svg>
              )}
            </button>
            <button onClick={onCerrar} className="p-2 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors">
  ...
  const handleMigrarFonasaNivel2 = async () => {
    ...
    try {
      ...
      for (const d of snap.docs) {
        const vp: ValorPrevision[] = d.data().valoresPrevision ?? [];
        const tieneNivel2 = vp.some(v => v.tipo === 'Fonasa Nivel 2');
        if (!tieneNivel2) continue;
        const tieneFonasa = vp.some(v => v.tipo === 'Fonasa');
        const nuevaVP = tieneFonasa
          ? vp.filter(v => v.tipo !== 'Fonasa Nivel 2')
          : vp.map(v => v.tipo === 'Fonasa Nivel 2' ? { ...v, tipo: 'Fonasa' } : v);
    ...
    try {
      ...
      for (const d of snap.docs) {
        ...
        if (prestacionInfo) {
          ...
          const index = valores.findIndex((v: any) => v.tipo === 'Fonasa');

          const newValor = {
            tipo: 'Fonasa',
            valor: prestacionInfo.total,
            copago: prestacionInfo.copago,
            bonificacion: prestacionInfo.bonificacion,
          };
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title="Actualizar precios de Fonasa masivamente basados en los códigos provistos"
              >
                Actualizar Fonasa Masivo
              </button>
              {prestaciones.some(p => p.valoresPrevision?.some(v => v.tipo === 'Fonasa Nivel 2')) && (
                <button
                  onClick={handleMigrarFonasaNivel2}
                  className="text-[10px] bg-red-100 text-red-700 px-2 py-0.5 rounded-full font-bold hover:bg-red-200 transition-colors"
                  title="Renombrar Fonasa Nivel 2 → Fonasa en todos los registros"
                >
                  Migrar Fonasa Nivel 2 → Fonasa
                </button>
              )}
            </div>
          </div>
      ...
      <div className="flex-1 p-6 overflow-hidden">
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col h-full max-h-[calc(100vh-220px)]">
          ...
          <div className="grid grid-cols-12 px-4 py-3 bg-slate-50 border-b border-slate-200 text-[11px] font-bold text-slate-500 uppercase tracking-widest">
            ...
            <span className="col-span-4">Fonasa (Total / Bono / Copago)</span>
            ...
            <div className="divide-y divide-slate-100 overflow-y-auto flex-1">
              {paginadas.map(p => (
                <div 
                  ...
                  className="grid grid-cols-12 px-4 py-3.5 hover:bg-slate-50 transition-colors group items-center cursor-pointer"
                >
                  ...
                  <span className="col-span-4 text-xs">
                    {(() => {
                      const fonasa = p.valoresPrevision?.find(v => v.tipo === 'Fonasa');
                  ...
                  <div className="col-span-1 flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                      ...
                      className="p-1.5 text-slate-400 hover:text-[#0E7490] hover:bg-[#0E7490]/10 rounded-lg transition-colors" 
                      title="Editar"
                    >
          ...
          {!loading && filtradas.length > 0 && (
            <div className="px-6 py-3 bg-slate-50 border-t border-slate-200 flex items-center justify-between flex-shrink-0">
              <div className="text-xs text-slate-500 font-medium">
                Mostrando <span className="text-slate-800 font-bold">{Math.min((currentPage - 1) * ITEMS_PER_PAGE + 1, filtradas.length)}</span> - <span className="text-slate-800 font-bold">{Math.min(currentPage * ITEMS_PER_PAGE, filtradas.length)}</span> de <span className="text-slate-800 font-bold">{filtradas.length}</span> resultados
              </div>
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className="p-2 rounded-xl border border-slate-200 text-slate-400 hover:text-[#0E7490] hover:border-[#0E7490]/20 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
                </button>
                
                <div className="flex items-center gap-1 px-2">
                  <span className="text-xs font-bold text-slate-700">Página {currentPage}</span>
