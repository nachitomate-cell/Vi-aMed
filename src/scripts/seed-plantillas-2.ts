...
const plantillas = [
  {
    prestacionLabel: 'ECOGRAFÍA ABDOMINAL',
...
No hay evidencias de ascitis, masas ni adenopatías retroperitoneales de carácter patológico.`,
  ...
  {
    prestacionLabel: 'ECOGRAFIA PARTES BLANDAS (REG. AXILAR)', // using a normalized likely name, will be matched
...
Vasos sanguíneos de trayectoria y calibre conservado.`,
  ...
  {
    prestacionLabel: 'ECOGRAFIA PARTES BLANDAS CADERA',
    hallazgos: `Se realiza barrido sonográfico con transductor de alta resolución en cadera X.
...
No se observan evidencias ecográficas de procesos expansivos sólidos.`,
  ...
  {
    prestacionLabel: 'ECOGRAFÍA DOPPLER ARTERIAL EXTREMIDADES INFERIORES',
...
Adecuada permeabilidad de tibiales posteriores, anteriores y pedias.`,
  ...
  {
    prestacionLabel: 'ECOGRAFÍA DOPPLER ABDOMINAL HEPATICO',
...
Grandes vasos de calibre y trayecto normales.`,
  ...
  {
    prestacionLabel: 'ECOGRAFÍA DOPPLER ABDOMINAL AORTOMESENTERICO',
...
Tronco celíaco y sus ramas esplénica y hepática, arteria mesentérica superior y arteria mesentérica inferior permeables con flujo presente. Las curvas presentan peaks sistólicos dentro del rango fisiológico.`,
  ...
  {
    prestacionLabel: 'ECOGRAFIA CUELLO',
...
Resto de estructuras exploradas sin alteraciones.`,
  ...
  {
    prestacionLabel: 'ECOGRAFIA PARTES BLANDAS CODO',
    hallazgos: `Con transductor de alta resolución se realiza barrido sonográfico en codo X.
...
async function seed() {
  try {
    ...
    const prestaciones = snap.docs.map(d => d.data());

    console.log('Encontradas', prestaciones.length, 'prestaciones en la DB.');

    for (const p of plantillas) {
      // Find matching prestacion
      const pNorm = makeKeyPlantilla(p.prestacionLabel);
      
      let matched = prestaciones.find(dbP => makeKeyPlantilla(dbP.nombre) === pNorm);
      
      // Fuzzy match if not found directly
      if (!matched) {
        matched = prestaciones.find(dbP => {
          const dbKey = makeKeyPlantilla(dbP.nombre);
          // check if they share a lot of words
          const wordsA = pNorm.split('_');
          const wordsB = dbKey.split('_');
          let count = 0;
          for(const w of wordsA) { if(wordsB.includes(w)) count++; }
          return count >= Math.max(wordsA.length - 1, 2);
        });
      }

      if (matched) {
        const key = makeKeyPlantilla(matched.nombre);
        console.log(`Asignando plantilla a: ${matched.nombre} (${key})`);
        await setDoc(doc(db, 'plantillas_eco', key), {
          prestacionLabel: matched.nombre,
          hallazgos: p.hallazgos,
          impresion: p.impresion,
          recomendaciones: ''
        }, { merge: true });
      } else {
        console.log(`NO SE ENCONTRO PRESTACION PARA: ${p.prestacionLabel}, insertando con key forzado...`);
        const key = makeKeyPlantilla(p.prestacionLabel);
        await setDoc(doc(db, 'plantillas_eco', key), {
          prestacionLabel: p.prestacionLabel,
          hallazgos: p.hallazgos,
          impresion: p.impresion,
          recomendaciones: ''
        }, { merge: true });
