...
exports.recuperarContrasena = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    if (req.method !== 'POST') {
...
function generarEmailConfirmacion(nombre) {
    return `
    <!DOCTYPE html>
    <html lang="es">
    <head>
...
exports.registrarProfesional = (0, https_2.onCall)(async (request) => {
    const { nombre, rut, email, password, rol, especialidad } = request.data;
    if (!email || !password || !rut || !nombre) {
        throw new https_2.HttpsError('invalid-argument', 'Faltan datos obligatorios');
    }
    try {
        ...
        throw new https_2.HttpsError('internal', error.message || 'Error al registrar profesional');
    }
});
